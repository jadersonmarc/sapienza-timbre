...
// createSession cria a sessão e reserva (lote + assentos). Sem auth. Taxa por IP + teto de
// sessões abertas por IP contra abuso de reserva anônima.
func (s *Server) createSession(w http.ResponseWriter, r *http.Request) {
	...
	err = s.withTenant(r.Context(), producerID, func(tx pgx.Tx) error {
		open, e := checkout.CountOpenByIP(r.Context(), tx, ip)
		if e != nil {
			return e
		}
		if open >= checkout.MaxOpenSessionsPerIP {
			return checkout.ErrTooManySessions
		}
		sess, e = checkout.CreateSession(r.Context(), tx, req, ip)
		if e != nil {
			return e
		}
		_, e = tx.Exec(r.Context(), `
			INSERT INTO public.checkout_session_index (session_id, producer_id, event_id)
			VALUES ($1,$2,$3)`, sess.ID, producerID, sess.EventID)
		return e
	})
	if err != nil {
		if errors.Is(err, checkout.ErrTooManySessions) {
			writeErr(w, http.StatusTooManyRequests, "muitas sessões abertas deste IP")
			return
		}
...
func (s *Server) patchSession(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.withTenant(r.Context(), producerID, func(tx pgx.Tx) error {
		...
		var e error
		sess, e = checkout.UpdateSession(r.Context(), tx, id, req)
		return e
	}); err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
		return
...
func (s *Server) bindSession(w http.ResponseWriter, r *http.Request, subjectID uuid.UUID) {
	...
	err = s.withTenant(r.Context(), producerID, func(tx pgx.Tx) error {
		return checkout.BindSession(r.Context(), tx, id, subjectID, s.authGrace)
	})
	switch {
